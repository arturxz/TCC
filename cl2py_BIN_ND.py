...
if __name__ == "__main__":
	...
	# INPUT IMAGE
	img_input = vl.VglImage("bin.pgm", vl.VGL_IMAGE_2D_IMAGE(), vl.IMAGE_ND_ARRAY())
	vl.vglLoadImage(img_input)
	vl.vglClUpload(img_input)

	img_input2 = vl.VglImage("bin2.pgm", vl.VGL_IMAGE_2D_IMAGE(), vl.IMAGE_ND_ARRAY())
	...
	vl.vglCheckContext(img_output, vl.VGL_RAM_CONTEXT())
	vl.vglSaveImage("bin-vglClNdBinDilatePack.pgm", img_output)

	wrp.vglClNdBinErode(img_input, img_output, window)
	vl.vglCheckContext(img_output, vl.VGL_RAM_CONTEXT())
	vl.vglSaveImage("bin-vglClNdBinErode.pgm", img_output)

	wrp.vglClNdBinErodePack(img_input, img_output, window)
	vl.vglCheckContext(img_output, vl.VGL_RAM_CONTEXT())
	vl.vglSaveImage("bin-vglClNdBinErodePack.pgm", img_output)

	wrp.vglClNdBinMax(img_input, img_input2, img_output)
	vl.vglCheckContext(img_output, vl.VGL_RAM_CONTEXT())
	vl.vglSaveImage("bin-vglClNdBinMax.pgm", img_output)

	wrp.vglClNdBinMin(img_input, img_input2, img_output)
	vl.vglCheckContext(img_output, vl.VGL_RAM_CONTEXT())
	vl.vglSaveImage("bin-vglClNdBinMin.pgm", img_output)

	wrp.vglClNdBinNot(img_input, img_output)
	vl.vglCheckContext(img_output, vl.VGL_RAM_CONTEXT())
	vl.vglSaveImage("bin-vglClNdBinNot.pgm", img_output)
